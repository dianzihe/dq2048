...
namespace PH
{
    ...
    */
	...
	public:
		...
		std::vector<Sprite*> comboTextList;

	public:
		BoardResult(/*HeroControlList& team*/) 
			//: baseAttack(team)
		{
		}
	...
	typedef shared_ptr<BoardResult> BoardResultPtr;
    static const int skBoardWidth = 6;
    static const int skBoardHeight = 5;
    
    class BoardControl;
    typedef shared_ptr<BoardControl> BoardControlPtr;
    typedef function<GemUtils::GemColor ()> ColorGenerator;
    ...
    public:
        ...
        {
            BoardControlPtr ret = make_shared<BoardControl>();
            if(ret && !ret->init(move(gen)))
                ret.reset();
            return ret;
        }
        ...
        TaskPtr removeGemFromGrid(const Vec2 & pos);
        TaskPtr removeGemFromGrid(const int pos);
        
        void removeGemFromGridRaw(GemPtr gem);
        void removeGemFromGridRaw(const Vec2& pos);
        void removeGemFromGridRaw(const int pos);

        TaskPtr fillBoard(ColorGenerator gemGen,
                          bool forceNoStacking = false);

        TaskPtr sweepBoard(/*PlayerControlPtr player,*/ BoardResultPtr result);
        
        TaskPtr patchGemGrid(GemColorGrid& patch);
        TaskPtr updateGemGrid();
        
        TaskPtr replaceGem(const Vec2& pos, GemPtr gem);
        TaskPtr replaceGemRaw(const Vec2& pos, GemPtr gem);
        
        void swapGem(const Vec2& left, const Vec2 right);
        
        // --- accessor --
        bool hasEmptyCell();
        inline GemGrid & getGrid() { return mGrid; }
        inline GemPtr operator() (const Vec2 & pos){ return mGrid(pos); }
        inline GemPtr get(const Vec2 & pos) { return mGrid(pos); }
    };
