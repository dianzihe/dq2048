...
namespace PH
{
    ...
    static Vec2 computeGemSetCenter(GemSet& set)
    {
        ...
        std::sort(sorted.begin(), sorted.end(), [](GemPtr a, GemPtr b)->bool
                  {
                      return (a->position.x < b->position.x) ||   // sort by x coord
                      (a->position.x == b->position.x && a->position.y < b->position.y); // if the same, sort by y corrd
                  } );
    ...
    static GemUtils::GemColor forceDifferentGem(const GemGrid& grid,
                                           const Vec2& p,
                                           function<GemUtils::GemColor ()> gemGen)
    {
        bool gemAllowed[GemUtils::AllColor];
        for(int i=0; i<GemUtils::AllColor; i++)
            gemAllowed[i] = true;
        
        Vec2 around[4] = { Vec2(p.x+1, p.y), Vec2(p.x-1, p.y),
            Vec2(p.x, p.y-1), Vec2(p.x, p.y+1) };
        
        for(const Vec2 p : around)
        {
    ...
    void BoardControl::vSerialize(rapidjson::Value& o, rapidjson::Document::AllocatorType& alloc) const
    {
        ...
        {
            GemPtr gem = mGrid(i);
			rapidjson::Value gemVal(kObjectType);
    ...
    void BoardControl::swapGem(const Vec2 &left, const Vec2 right)
    {
        GemPtr a = get(left);
        GemPtr b = get(right);
    ...
    void BoardControl::removeGemFromGridRaw(const Vec2& pos)
    {
        if(pos.x < 0 || pos.x >= mGrid.Width ||
           pos.y < 0 || pos.y >= mGrid.Height)
            return;
    ...
    TaskPtr BoardControl::removeGemFromGrid(const Vec2 & pos)
    {
        if(pos.x < 0 || pos.x >= mGrid.Width ||
           pos.y < 0 || pos.y >= mGrid.Height)
...
//                    result->baseAttack.healLabel = player->createHealLabel(this->root);
    ...
    {
        assert(patch.Width == mGrid.Width && patch.Height == mGrid.Height && "Grid size mismatch");
    ...
    TaskPtr BoardControl::replaceGem(const Vec2& pos,GemPtr now)
    {
        if(pos.x < 0 || pos.x >= mGrid.Width ||
           pos.y < 0 || pos.y >= mGrid.Height)
            return TaskIgnore::make();
    ...
    TaskPtr BoardControl::replaceGemRaw(const Vec2 &pos, GemPtr now)
    {
        if(pos.x < 0 || pos.x >= mGrid.Width ||
           pos.y < 0 || pos.y >= mGrid.Height)
            return TaskIgnore::make();
        ...
        for(int col=0; col<GemUtils::GemColorMax; col++)
        {
            ...
            if(backup)
            {
                if(ret.val[col] == 2)
                {
                    ...
                    {
                        path.push_back(pos);
                        path.push_back(pos+vec.x.first);
                        
                        mask(pos+vec.x.first) = true;
                        mask(pos+vec.y.first) = true;
                        
                        ret.is_potential_point = true;
                        ret.potential_color = grid(pos+vec.x.first);
                        ret.potential_dir = vec.x.first;
                        ret.pattern = 3;
        ...
        for(int i=0; i<grid.size(); i++)
        {
            ...
            for(float index=0; index<maxCombo; index++)
            {
                ...
                for(int nowPos=0; nowPos<grid.size(); nowPos++){
                    ...
                    if(val.is_potential_point)
                    {
                                                    ...
                                                    temp_mask, move))
                        {
                            ...
                            if(footprints(move.x) && footprints(path.back()))
                                continue;
                ...
                if(path.size()>0 && shortest.size() >0)
                    std::swap(copyGrid(path.back()), copyGrid(shortest.x));
