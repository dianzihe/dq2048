...
public:
	...
	*/
    ...
    GenericValue& AddMember(const Ch* name, const std::string& value, Allocator& allocator) {
		GenericValue n(name, internal::StrLen(name));
		GenericValue v(value.c_str(), value.size(), allocator);
...
\endcode
	...
	*/
	...
	GenericValue& PushBack(const std::string& value, Allocator& allocator) {
		GenericValue v(value.c_str(), value.size());
		return PushBack(v, allocator);
	}
